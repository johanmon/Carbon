...
in average half of our yearly emissions. If we look at the
...
The yearly increase is not as uniform as one would suspect and, as can
...
which are large carbon reservoirs. Carbon dioxide in the atmosphere is
...
to make prediction of all reservoirs as long as it helps us make
...
vegetation, volcanoes, oceans etc. Since the earth surface to 70\%
consist of water we can, for simplicity, call this box {\em the
...
interchange between these ocean layers is described both by {\em
  diffusion} and {\em advection} i.e. ocean currents. These model
however, aspire to also describe what is happening in the biosphere
and oceans, not only what is happening in the atmosphere. If we are
only interested in the atmosphere, a simple two-box model might be
sufficient.
...
carbon in the atmosphere in realtion to the oceans, and the flow in
both directions. If we, for sake of the argument, assume that the
amount of carbon in the oceans is fifty times the amount in the
atmosphere, then we can predict what will happen with 10 GtC (gigaton of
carbon) that we emit during a year. Eventually the 10 GtC will, as
all carbon, be distributed in the ratio 1:50. How long time this will
take depends on the flow between the reservoirs. If 10\% of the carbon
in the atmosphere enters the oceans each year, and $0.2\%$ of the
carbon in the oceans enters the atmosphere, we will see a decrease of
the extra carbon in the atmosphere and after some decades we will find
some $0.2$ GtC still in the atmosphere and the remaining $9.8$ GtC
gigaton in the oceans.

Of the parameters the only one that we can know with some certainty is
...
size of the oceans is normally given as around $40.000$ GtC so our
example was probably not that far of reality. The transport to and
from the atmosphere is much harder to estimate but most sources would
agree on a number in the hundreds of GtC if both ocean and vegetation
is included.
...
when they hit nitrogen atom will convert these atoms to a carbon-14
isotope (14C). Carbon consists almost solely of 12C and only one
isotope in a trillion is a 14C isotope. The atmosphere thus only
contains less than a ton of 14C but in the late fifties until 1965
this almost doubled. We did thus actually "color" the carbon in the
atmosphere and now have the result.
...
compiled by Quan Hua Chan et.al \cite{Hua:22} and is based on
atmospheric sampling and tree rings. The concentration is estimated to
have dropped by five percent by 1950 due to the so called {\em Suess
  effect}.  Carbon form fossil sources is free of 14C since the isotop
is radioactive and slowly decays to 12C.  As 14C free carbon from
fossil emissions is added to the atmosphere, the concentration of 14C
decrease.

As is clearly seen the 14C concentration rapidly goes up during the
bomb testings and then reaches a peek in 1965 (bomb generated 14C
initially affected the stratosphere and it took two years for it to be
evenly distributed in the troposphere). The rapid decline is apparent
- carbon dioxide in the atmosphere is quickly mixing with other
reservoirs and the excess 14C quickly goes away. However, when we
interpret these numbers we need to be careful - what is measured is
the concentration of 14C - not the total amount of 14C.

To estimate the amount of 14C we need to take into account that the
atmosphere now contains more carbon dioxide compared to the mid
...
show how the total amount of carbon dioxide has grown since 1955. The
14C curve in in Fig. \ref{fig:relative55} shows how the bomb
experiments quickly raise the amount of 14C in the atmosphere to
$75\%$ above the 1955 levels. It then quickly decreases until 1994
where it levels off and then slowly increase. Any physical model that
we build should be able to recreate this process.
...
Fig. \ref{fig:terms}. We now see that the exponential decay continues
...
values are even smaller. The small and random distribution of the
residual values indicate that the statistical model captures the
dominating behavior.
...
The two terms of the best fit equation could be explained by a model
where oceans are a factor fifty larger than the athmosphere and $1/17
\approx 6\%$ of the carbon in the atmosphere is transported to the
oceans every year. Our task is to create a physical model that shows
the same behavior. This will as we shall see to be a quite tricky
task.
...
free from 14C. The Suess effect is very much present in the period
covered by the bomb curve. However, when we goo into the details we
have more things to consider.

We have a small contribution of 14C to the atmosphere each year. One
source i natural galactic radiation that creats C14 from
...
carbon in the oceans. The assumptions thus are:
\begin{itemize}
\item atmosphere is well mixed,
\item oceans are well mixed,
\item flow from atmosphere to oceans proportional to concentration in atmosphere and,
\item flow from oceans to atmosphere  proportional to concentration in oceans.
\end{itemize}

We know that these assumtions are over simplifications. The atmosphere
can of course be treated as well mixed sin ce we are only modeling
yearly changes but the oceans are far from well mixed. If, however, we
can create a simple model that can explain past events and predict the
future there is no point in creating a more complex model. Our model
is thus a simple two-box model where the fossil emissions are given
and differential equation determines the flows between reservoirs. 
...
and the left is to determine the size of the ocean and the rate of flow
...
If we zoom in we find that the lowest mean squared error is found when
...
Of the two results the second is clearly the one we shoudl favor. It
does not explain the growth of carbon in the atmosphere as well as the
first result but it is not that far off. The first result might be a
better at describing the growth of carbon in the atmosphere but as
seen in \ref{fig:bomb-fit} it fails in explaining the 14C
concentrations.
...
The question now is if this model also can explain the increase that
we have seen since 1850.

\subsection*{growth since 1850}

A sanity check would be to see how well the physical model describes
the growth of carbon dioxide in the atmosphere since 1850. We do not
have unbroken measurements series that dates back to 1850 but we have
proxies in the form of ice cores drilled at Antarctica. According to
the ice core studies the concentration of carbon dioxide in the
atmosphere was around 283 ppm in 1850. We also have good estimates on
fossil fuel consumption since 1850 so we can run a simulation and see
how well the numbers match.

\begin{figure}
   \centering
   \includegraphics[scale=0.5]{img/law_dome.pdf}
   \caption{Simulated growth of carbon dioxide in atmosphere since 1850.}
   \label{fig:law_dome}
\end{figure}


The result can be seen in Fig. \ref{fig:law_dome}. As clearly seen the
model underestimates the historical growth, both as derived from the
ice cores and the values as measured at Manual Loa. We could of course
adjust the parameters of the model but we would then divert from the
best adjustment to the measured values. So either the ice core data is
wrong, our simple model is wrong or both are wrong.

\section*{Temperature dependence}

Of the assumtions we made 
